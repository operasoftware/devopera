...
Opera now supports the [Web Notifications API](http://dvcs.w3.org/hg/notifications/raw-file/tip/Overview.html), available under the `window.Notification` object. Even though the Chromium project has supported this feature for a long time, we wanted to get the UX right before shipping this in Opera.
