...
한마디로 하드웨어 가속이란 그래픽 처리 장치(GPU)를 이용하여 중앙 처리 장치(CPU)의 처리량을 줄이고, 브라우저의 렌더링을 효율화하는 것을 말한다. 하드웨어 가속을 활성화하고 CSS를 사용하면 렌더링을 빠르게 처리하고 페이지의 출력을 고속화할 수 있다.
...
CSS animation, transform, transition 속성은 GPU 가속이 자동으로 활성화되지 않는다. 이 속성들은 브라우저에 탑재돼 있는 다소 동작이 느린 렌더링 엔진에 의해 실행된다. 하지만 일부 브라우저에는 렌더링 처리 능력을 향상시킬 수 있는 몇가지 프로퍼티가 구현돼 있으며 이를 이용해 하드웨어 가속을 활성화(High Performance Animations)할 수 있다. 예를 들면 opacity다. 이 속성은 GPU에 의해 효율적으로 처리되기 때문에 고속으로 처리될 수 있는 몇 안되는 프로퍼티 중 하나다. CSS transition이나 animation에서 투명도를 변경하려는 레이어가 있는 경우 브라우저는 그 엘리먼트의 처리를 GPU에게 담당하도록 넘긴다. 따라서 렌더링이 정말 빠르게 처리된다. CSS의 프로퍼티 중에서 opacity는 아주 유용한 도구이며 이 프로퍼티를 사용하는데 큰 문제는 없을것이다. 기타 하드웨어 가속이 가능한 프로퍼티로는 CSS 3D transforms가 자주 사용된다.
...
오랜시간동안 우리들은 translateZ(또는 translate3d)라고 불리는 CSS핵(때때로 null transforms 핵이라 불리는)을 이용해 렌더링을 고속화했다. 이것은 브라우저를 속여 강제로 animation이나 transform의 하드웨어 가속을 활성화시키는 방법이다. 3차원적 변형이 필요하지 않은 엘리먼트에 단순히 3D 변형을 지시를 하여 렌더링 처리를 고속화한다. 예를 들어 2차원 공간에서 애니메이션되는 엘리먼트에 아래와 같은 단순한 CSS 코드를 작성함으로써 하드웨어 가속을 활성화할 수 있다.
...
이러한 방법으로 하드웨어 가속을 행하면 합성 레이어라는 것이 생성된다. 이것은 GPU 위에 올려져 GPU에 의해 합성되는 레이어다. 하지만 CSS 핵을 이용해 생성한 레이어는 성능 병목을 해소하는데 항상 도움이 되지는 않는다. 합성 레이어를 생성하는 것은 페이지 출력 속도를 빠르게 할지는 모르겠지만 그만한 비용이든다. RAM이나 GPU의 메모리 사용량이 커지며 레이어를 많이 생성하면 할수록 그만큼 악영향을 끼친다(특히 모바일에서). 따라서 핵을 사용할때는 신중하게 다뤄야한다. 하드웨어 가속의 실행에 의해 정말 페이지의 출력이 빨라지는지 그리고 이로인한 또다른 성능 병목이 발생하지 않는지를 확인한 상태에서 테크닉을 사용해야만 한다.
...
레이어 생성을 증가시키는 CSS 핵을 대신할 수 있는 새로운 CSS 프로퍼티가 도입됐다. 이 프로퍼티는 엘리먼트에 변경을 하기전에 그것이 어떤 변경인지를 브라우저에 알리는 효과가 있는데 이 프로퍼티로 브라우저는 특정 엘리먼트에 조작을 가하기 전에 최적화할 수 있게 된다. 예를들어 애니메이션과 같은 비용이 필요한 처리를 애니메이션이 실제로 시작되기 전에 준비할 수 있다는 뜻이다. 이 프로퍼티가 바로 `will-change`다.

## 새로운 방법 : 끝내주는 will-change 프로퍼티
...
엘리먼트에 어떠한 변경을 할 것인지를 미리 브라우저에 알려주는 것이 `will-change` 프로퍼티의 역할이다. 이것을 사용하면 그 변경이 시작되기전에 적절히 최적화할 수 있다. 즉, 페이지 출력에 악영향을 줄 수 있는 처리 비용을 줄일 수 있다는 것이다. 그로인해 효율적으로 엘리먼트의 변경 또는 렌더링을 처리할 수 있고 페이지는 순식간에 갱신돼 부드러운 화면 처리가 가능하게된다.
...
CSS 3D Transforms를 예로 들어보겠다. 「CPU, GPU, 하드웨어 가속」절에서 말했듯이 이 프로퍼티를 어느 특정 엘리먼트에 사용하면 그 엘리먼트와 컨텐츠는 레이어로 관리되고 나중에 다시 합쳐진다. 새로운 레이어로 엘리먼트를 분리하는 것은 비교적 비용이 필요한 작업이다. 따라서 애니메이션에 몇 분의 1초 단위의 눈에 띄는 지연 현상이 발생한다. 이것이 화면에서 깜빡이는 현상으로 이어지는 것이다.
...
`will-change` 프로퍼티를 사용해 앞으로 일어날 변경에 관해 브라우저에게 알려주고자 할때는 대상이 되는 엘리먼트에 아래와 같이 CSS 코드를 작성하면 된다.
...
변형 처리 외에도 스크롤 위치(표시되고 있는 윈도우 내의 엘리먼트 위치, 화면 내에 엘리먼트가 얼마나 보이고 있는지), 컨텐츠 등 기타 복수의 CSS 프로퍼티를 변경할 때에는 그 대상이 되는 프로퍼티 명을 지정하여 브라우저에 변경 의사를 전달할 수 있다. 1개의 엘리먼트에 여러개의 값을 변경할 생각이라면 콤마(,)로 구분하여 기술할 수 있다. 예를들어 한 엘리먼트를 애니메이션 시키는 동시에 위치를 변경하고자하는 경우는 다음과 같이 선언하면 된다.
...
### will-change는 엘리먼트의 변경을 브라우저에 알려주는 이외에 엘리먼트 자체에도 어떠한 영향을 주는가?
...
이 질문에 관한 답변은 예스 또는 노다. 변경할때 사용하는 프로퍼티의 종류에 따라 상황이 달라진다. 엘리먼트에 [스택 컨텍스트](http://reference.sitepoint.com/css/stacking)([참고](https://www.w3.org/TR/CSS2/zindex.html))를 생성하는 초기화하지 않은 프로퍼티가 있다면 그것을 `will-change`에 지정하는 것으로 엘리먼트에 스택 컨텍스트가 생성될 수 있다.
...
예를들어 clip-path와 opacity 프로퍼티는 모두 기본 값 이외의 값을 지정할 때 대상이 되는 엘리먼트에 스택 컨텍스트를 생성한다. 즉, will-change의 값으로 두 프로퍼티(혹은 모두)를 지정하면 엘리먼트에 관한 실제 변경이 발생하기 전(즉, opacity 기본 값을 변경하기 전)에 그 엘리먼트에 스택 컨텍스트가 만들어지는 것이다. 이와 같은 원리는 엘리먼트에 스택 컨텍스트를 생성하는 또다른 프로퍼티 역시 동일하게 적용된다.
...
또, 일부 프로퍼티는 위치가 고정된 엘리먼트(fixed-position elements)에서 컨테이닝 블럭(containing block) 생성을 일으킨다. 예를들면 변형 처리된(transformed) 엘리먼트는 위치가 고정(position: fixed)돼 있더라도 모든 자식 엘리먼트에 대해 컨테이닝 블럭을 생성한다(참고). 즉, 컨테이닝 블럭의 생성을 발생시키는 프로퍼티를 will-change의 값으로 지정한 경우 위치가 고정된 엘리먼트에 컨테이닝 블럭이 만들어진다는 뜻이된다.
...
앞서 설명한 스택 컨텍스트와 컨테이닝 블럭, 두 가지 예외를 제외하면 `will-change` 프로퍼티는 대상 엘리먼트에 직접적인 영향을 끼치지 않는다. `will-change`는 브라우저에 앞으로 일어난 변화를 알려줘 최적화를 실시할 뿐이다.
...
`will-change`의 역할을 이해하게 되면서 "브라우저에 모든 것을 최적화하면 좋지 않을까"라고 생각할 수도 있을 것이다. 누구라도 모든 변경에 관한 최적화가 한번에 됐으면 좋겠다고 생각할 수 있다.
...
확실히 `will-change`는 파워풀하고 훌륭한 도구지만, 또다른 훌륭한 도구들과 마찬가지로 위력이 있는 만큼 책임을 가지고 다뤄야한다. `will-change`는 현명하게 사용해야한다. 그렇게 사용하지 않으면 퍼포먼스 히트가 일어나고 페이지가 크래쉬될 것이다.
...
`will-change`는 퍼포먼스 히트를 비롯, 바로 감지(directly detectable)되지 않는 사이드 이펙트(원래 will-change는 보이지 않는 곳에서 브라우저에 명령하는 방법이므로 감지하지 못하는 건 당연하다)를 발생시키기 때문에 사용하기 까다로운 프로퍼티다. 이 프로퍼티를 사용해 최대한 효과를 발휘하고 발생할 수 있는 문제를 피하기 위해서는 아래 몇 가지 규칙을 이해해야한다.

### 필요 이상의 프로퍼티 선언이나 엘리먼트, will-change를 사용하고 변경을 선언하지 않는 일
...
이전 절에서도 말했듯이 모든 프로퍼티, 모든 엘리먼트에 대해 일어날 변경을 전부 브라우저에 최적화 처리를 하려한다면 CSS로 다음과 같이 작성할 수 있다. 언뜻 보기엔 그렇게 틀려보이지 않을 수 있다.
...
즉, 일어날지 일어나지 않을지 모르는 변경에 대비해 브라우저에 최적화 시키는 것은 현명하지 못하고 효과가 없을 뿐더러 문제를 발생시키니 하지말자.
...
`will-change` 프로퍼티의 명칭의 유래에는 명확한 이유가 있다. `will-change`가 브라우저에게 알려주는 것은 현재 일어날 변화가 아닌 미래에 일어날 변화다. `will-change`를 사용하여 브라우저에게 앞으로 일어날 변경에 관해 최적화를 시키는 것이기 때문에 이를 위해 브라우저도 최적화를 위한 시간이 필요하다. 이는 실제로 변화가 생길 때 지연 없이 최적화를 적용할 수 있도록 하기 위함이다.
...
예를들어 엘리먼트가 클릭되는 순간 변화된다 할 때 호버 시 `will-change`를 설정하면 브라우저가 변경에 관해 최적화하는 시간을 벌 수 있다. 사용자가 엘리먼트에 호버한 후 실제 클릭할 때까지의 시간 차는 브라우저가 충분히 최적화를 실시할 수 있는 시간이기 때문이다. 인간의 반응은 비교적 시간이 걸리기 때문에 실제 변경이 생기기 전 까지 약 200밀리세컨드의 시간이 브라우저에게 주어진다. 브라우저가 최적화를 하는데는 그만큼 시간이면 충분하다.
...
그러면, 선택이 아니라 호버로 변경시키고 싶은 경우는 어떻게할까? 아까도 말했듯이 처음 선언한 코드는 소용이없다. 그러나 이 경우에도 변경이 발생하기 전에 예측할 수 있는 방법을 찾아낼 수 있다. 예를 들어, 변경하는 엘리먼트의 조상에 호버하면 리드타임(lead time)을 벌 수 있다.
...
하지만, 조상에 호버 한다고 해서 대상의 엘리먼트가 반드시 상호 작용(interacted)된다고는 말할 수 없다. 그러므로 애플리케이션에서 뷰가 액티브될 때나 엘리먼트가 viewport에 보이는 위치에 있을 때 `will-change`를 설정하면 엘리먼트가 상호 작용할 가능성을 높일 수 있다.
...
브라우저가 앞으로 발생할 변화에 관해 최적화를 하면 일반적으로 비용이 든다. 「필요 이상의 프로퍼티 선언이나 엘리먼트, will-change를 사용하고 변경을 선언하지 않는 일」 절에서 잠깐 이야기했듯이 머신의 자원을 대량으로 소비하는 일이 있기 때문이다. 브라우저는 보통, 적용한 최적화를 삭제하고 가능한한 빨리 평소의 행동(nomal behavior)으로 돌아온다. 하지만 `will-change`는 이 행위를 무시하고 본래 브라우저가 하는 것보다 훨씬 길게 최적화를 유지해버린다.
...
스타일시트에 선언된 `will-change`는 삭제할 수 없다. 따라서 대부분 자바스크립트를 사용해 설정 및 삭제하도록 권장한다. 스크립트에서 브라우저에 변경을 선언하고 그 변경이 종료될 때 즈음에 이벤트를 등록하면 변경이 종료된 후 `will-change`를 삭제할 수 있다. 예를들어 앞 절에서 소개한 스타일 규칙과 동일하게 엘리먼트(또는 그 조상)에 `mouseenter` 이벤트를 이용해 마우스가 호버될 때를 리슨하여 `will-change`를 설정할 수 있다. 엘리먼트를 애니메이션하는 경우엔 DOM 이벤트의 animationEnd를 사용해 애니메이션이 종료할 때를 리슨하여 animationEnd가 발생했을 때 will-change를 삭제한다.
...
`will-change` 프로퍼티는 4개의 설정 가능한 값(auto, scroll-position, contents, <custrom-ident>)을 제공한다.
...
`<custom-ident>`의 값으로는 변경하고 싶은 1개 이상의 프로퍼티 명을 지정한다. 프로퍼티가 복수인 경우엔 콤마(,)로 구분한다. 아래 코드는 프로퍼티 명으로 지정한 유효한 `will-change` 선언이다.
...
앞에서 말했듯이 `will-change`를 지정해도 아무 영향을 받지 않는 프로퍼티도 있다. 왜냐하면 브라우저는 대부분의 속성 변화에 관해 어떤 특별한 최적화를 행하지 않기 때문이다. 그러한 프로퍼티를 will-change에 지정해도 안전하지만 별다른 영향도 주지 않는다. 기타 속성은 스택 컨텍스트(opacity, clip-path 등)나 컨테이닝 블럭 또는 양쪽 모두 생길 수 있다.
...
will-change 프로퍼티는 핵 불필요한 최적화 코드를 실현하고 CSS 조작 속도와 성능의 중요성을 돋보이게 한다. 그러나 다른 모든 도구와 마찬가지로 큰 힘을 가진 도구는 큰 책임이 뒤따른다는 사실을 기억해야한다. `will-change`는 너무 가볍게 접근하지 않고 현명하게 이용해야할 프로퍼티 중 하나다. 여기에서 [`will-change` 사양](https://drafts.csswg.org/css-will-change/#recently-viewed)의 에디터인 탭 애킨스 주니어(Tab Atkins Jr)의 말을 인용한다.
...
> will-change는 실제로 변화시키는 프로퍼티, 실제로 변화가 발생할 엘리먼트에 설정하라. 그리고 변화가 종료되면 삭제하라.
