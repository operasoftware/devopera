...
You may be familiar with [`<a rel=nofollow>`](https://html.spec.whatwg.org/#link-type-nofollow), which prevents sending the `Referer` HTTP header and sets `window.opener` to `null` when following a link.

If you only want to disable `window.opener` but still send the `Referer` header, you can now use [`rel=noopener`](https://html.spec.whatwg.org/#link-type-noopener). Note that this is only needed for links that open a new navigation context, i.e. `target="_blank"`.
